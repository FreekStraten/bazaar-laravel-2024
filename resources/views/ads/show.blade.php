                                ...
                                <table class="table-auto w-full mb-6 rounded-lg overflow-hidden">
                                    ...
                                    @foreach ($ad->bids as $bid)
                                        <tr class="odd:bg-gray-100 dark:odd:bg-gray-600 even:bg-white dark:even:bg-gray-800 rounded-b-lg">
                                            ...
                                            <td class="@if($bid->is_accepted) underline @endif border px-4 py-2">
                                                €{{ $bid->amount }}</td>
                                            ...
                                            @if(auth()->id() == $ad->user_id)
                                                <th class="border px-4 py-2">
                                                    @if ($bid->is_accepted)
                                                        <x-secondary-button
                                                            disabled>{{ __('ads.accept_bid') }}</x-secondary-button>
                                                    @else
                                                        <form
                                                            action="{{ route('ads.accept-bid', [$ad->id, $bid->id]) }}"
                                                            method="POST">
                                                            @csrf
                                                            <x-primary-button
                                                                type="submit">{{ __('ads.accept_bid') }}</x-primary-button>
                                                        </form>
                                                    @endif
                                                </th>
                                            @endif
                ...
                <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                    <div class="p-6 text-gray-900 dark:text-gray-100">
                        @if($ad->bids->where('is_accepted', true)->first()->pickup_date && $ad->bids->where('is_accepted', true)->first()->return_date)
                            ...
                            @if(app()->getLocale() == 'nl')
                                <p>{{ __('ads.pickup_date') }}
                                    : {{ \Carbon\Carbon::parse($ad->bids->where('is_accepted', true)->first()->pickup_date)->format('d-m-Y') }}</p>
                                <p>{{ __('ads.return_date') }}
                                    : {{ \Carbon\Carbon::parse($ad->bids->where('is_accepted', true)->first()->return_date)->format('d-m-Y') }}</p>
                            @else
                                <p>{{ __('ads.pickup_date') }}
                                    : {{ \Carbon\Carbon::parse($ad->bids->where('is_accepted', true)->first()->pickup_date)->format('d-m-Y') }}</p>
                                <p>{{ __('ads.return_date') }}
                            ...
                            <form action="{{ route('ads.set-dates', $ad->id) }}" method="POST">
                                ...
                                <div class="mb-4">
                                    <label for="pickup-date"
                                           class="block font-medium text-gray-700 dark:text-gray-300">{{ __('ads.pickup_date') }}</label>
                                ...
                                <div class="mb-4">
                                    <label for="return-date"
                                           class="block font-medium text-gray-700 dark:text-gray-300">{{ __('ads.return_date') }}</label>
