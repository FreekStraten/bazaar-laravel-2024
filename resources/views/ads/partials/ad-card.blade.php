{{-- resources/views/ads/partials/ad-card.blade.php --}}
@props(['ad'])
...
<div class="card relative">
    ...
    <div class="absolute right-2 top-2 flex items-center gap-1">
        ...
        <form method="POST" action="{{ route('favorites.toggle', $ad->id) }}"
            ...
            @csrf
            <button type="submit" title="{{ $isFavorite ? __('Unfavorite') : __('Favorite') }}"
                    class="chip" onclick="event.preventDefault(); this.closest('form').submit();">
                @if($isFavorite)
                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"
                         fill="currentColor" class="w-5 h-5" style="color: rgb(var(--accent));">
